(function($, window, undefined) {
  ...
  (function(gestures, state, calc, util) {
    ...
    var config = {
      ...
      distance, prevTapPos, prevTapEndTime, holdTimer;
  ...
  (function(gestures, state, calc) {
    ...
    function transform(event) {
      var rotation, scale, $target = $(event.target);
      ...
      if (state.gesture === 'transform' || Math.abs(1 - scale) > config.scale_treshold || Math.abs(rotation) > config.rotation_treshold) {
        ...
        center = {
          pageX: ((state.touches.move[0].pageX + state.touches.move[1].pageX) / 2) - state.offset.left,
